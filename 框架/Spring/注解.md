...
- `@Mapper public interface UserMapper {}`：标识这是 MyBatis 的 Mapper 接口
- `@MapperScan("com.example.mapper")`：批量扫描 Mapper，一般写在启动类上面
- SQL 类注解（写在方法上，简单 SQL ，不写 xml）：
  - `@Select`，`@Insert`，`@Update`，`@Delete`
- `@Param`：给参数命名（XML / 注解SQL中用）
- `@TableName`：指定表名
- `@TableId`：主键策略
  - AUTO（自增）
  - ASSIGN_ID（雪花算法，默认）
