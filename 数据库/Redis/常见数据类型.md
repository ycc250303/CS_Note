...
* Redis 保存键值对涉及到的数据结构
  ...
  * 数组中的每个元素都是指向一个哈希表节点结构（dictEntry）的指针；**key 指向的是 String 对象，而 value 则指向类型对象**

![1772511531265](image/常见数据类型/1772511531265.png)
...
![1772511614941](image/常见数据类型/1772511614941.png)
...
* 字符串对象内部编码：int、raw 和 embstr
  ...
  ![1772508780712](image/常见数据类型/1772508780712.png)

  * 整数，可以用long表示：int 编码

    ![1772508791007](image/常见数据类型/1772508791007.png)
...
* embstr 编码的好处
  ...
  * 内存分配和释放从两次减少到一次
  * 字符串对象所有数据连接在一块连续的内存里面，更好利用CPU提升性能
* 缺点
...
* Redis 5.0 新增，专门为消息队列设计的数据类型
* 支持消息持久化、自动生成全局唯一 ID、ACK 确认、消费组
* List 做队列有两个缺口：生产者要自己造全局唯一 ID；消息被取出后就删掉，不支持消费组。Stream 把这两点补上了

### 常用命令

- XADD：插入消息，保证有序，可以自动生成全局唯一 ID；可带 `MAXLEN` 限制队列长度
- XLEN：查询消息长度；
- XREAD：用于读取消息，可以按 ID 读取数据；可带 `BLOCK` 做阻塞读
- XDEL：根据消息 ID 删除消息；
- DEL：删除整个 Stream；
- XRANGE：读取区间消息；
- XGROUP：创建 / 销毁消费组
...
* 消息队列：XADD + XREAD 就能做简易队列；消费组 + ACK 才是完整方案

  * 生产消息：`XADD`，`*` 表示让 Redis 自动生成 ID。ID 格式是 `毫秒时间戳-序号`，同一毫秒内从 0 递增，天然有序、全局唯一，消费者可以靠这个 ID 做去重

    ```sql
    # * 表示自动生成全局唯一 ID；返回值就是这条消息的 ID
    > XADD mymq * name xiaolin
    "1654254953808-0"

    # MAXLEN 限制队列长度，超出后删掉最旧的消息，避免内存被积压撑爆
    > XADD mymq MAXLEN 1000 * name xiaolin
    ```
  * 消费消息：`XREAD` 从指定 ID 的**下一条**开始读（不是读这条本身）。`$` 表示只跟最新消息；`BLOCK` 毫秒数做阻塞读，没消息就挂起，类似 List 的 `BRPOP`，省掉空转轮询

    ```sql
    # 从该 ID 的下一条开始读后续消息
    > XREAD STREAMS mymq 1654254953807-0

    # $ 表示读最新；BLOCK 10000 表示最多阻塞 10 秒
    > XREAD BLOCK 10000 STREAMS mymq $
    ```
  * 消费组：用 `XGROUP CREATE` 建组，再用 `XREADGROUP` 消费。**同一组内**一条消息只会被一个消费者读到，用来做负载分担；**不同组**可以从相同起点各自消费同一条消息（创建时要指定相同起始位置）
    ...
    # COUNT 限制每次读几条，组内多个消费者各拿一部分
    > XREADGROUP GROUP group2 consumer1 COUNT 1 STREAMS mymq >
    > XREADGROUP GROUP group2 consumer2 COUNT 1 STREAMS mymq >
    ```
  * 可靠性：消费者读走后，消息会进该组的 **PENDING List**，直到发送 `XACK`。宕机重启后用 `XPENDING` 把未确认的捞回来继续处理。业务逻辑跑完再 ACK，避免处理失败丢消息。**XACK 只从 PENDING List 删掉这条记录，Stream 里的原文还在**，真正删消息靠 `XDEL` 或超 `MAXLEN` 被裁掉

    ```sql
    # 查看 group2 里已读取但尚未 ACK 的消息概况
    > XPENDING mymq group2

    # 查看某个消费者具体 pending 了哪些消息
    > XPENDING mymq group2 - + 10 consumer2

    # 处理完成后确认，消息从 PENDING List 移除
    > XACK mymq group2 1654256265584-0
    ```
  * 三大需求怎么满足
    * 消息保序：`XADD` / `XREAD`
    * 阻塞读取：`XREAD BLOCK`
    * 处理重复消息：`XADD` 自动生成全局唯一 ID
    * 保证消息可靠性：PENDING List + `XPENDING` + `XACK`
    * 消费组：`XGROUP` + `XREADGROUP`
* 和专业消息队列区别

  * 消息不丢要看三个环节
    * 生产者：看有没有处理好 `XADD` 的返回值和异常，失败就重发
    * 消费者：PENDING List + `XACK`，重启能续上，一般不丢
    * Redis 自身会丢：AOF 每秒刷盘是异步的，宕机可能丢约 1 秒数据；主从复制也是异步，主从切换也可能丢。Kafka / RabbitMQ 通常多副本落盘，中间件环节更稳
  * 消息可堆积：数据在内存，积压会顶内存、有 OOM 风险。`XADD` 带 `MAXLEN` 后超长就删旧消息，等于用丢旧消息换内存安全。专业 MQ 落磁盘，积压主要占磁盘
  * 适不适合当 MQ
    * 场景简单、丢一点无所谓、积压概率小：可以
    * 海量消息、容易积压、不能丢：用专业 MQ
* 发布/订阅为什么不能当消息队列
  * 不基于数据类型，相关操作不写 RDB / AOF，Redis 重启数据全丢
  * 发后即忘，订阅者离线重连拿不到历史消息
  * 消费端积压超过 `client-output-buffer-limit pubsub`（默认 32MB，或 60s 内持续 8MB 以上）会被强制断开
  * 适合即时通知，比如哨兵集群内部通信
