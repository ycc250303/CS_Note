...
以 `UPDATE t_user SET name = 'xiaolin' WHERE id = 1;` 为例。三种日志怎么配合，见下面「update 语句执行过程」；每种日志的细节仍按 undo / redo / binlog 分节。
...
* autocommit 默认开启，单条增删改也会隐式开事务，执行完自动提交。没提交就崩溃，靠 undo log 把数据滚回去
* 不同操作记下的内容不一样
  * insert：记下主键，回滚时按主键删掉
  * delete：记下整行内容，回滚时再 insert 回去。真正删除是打 delete flag，purge 线程稍后清
  * update 非主键列：记下被改列的旧值，回滚时再 update 回去
  * update 主键列：先删后插
* undo 页在 Buffer Pool 里，改 undo 页同样要记 redo log。undo 和数据页一样，靠 redo 刷盘保证持久，没有单独的 undo 刷盘策略
...
* 一个事务的 binlog 不能拆开写。提交时执行器把该线程 binlog cache 里的完整事务一次性写入 binlog 文件，再清空 cache。超 `binlog_cache_size` 会先落到临时文件
* write 只写到操作系统 Page Cache，fsync 才真正落盘。`sync_binlog=0` 性能最好、主机异常重启可能丢还在 Page Cache 里的 binlog；`=1` 最安全但每次提交都 fsync；能接受丢少量事务时可设 100~1000

## update 语句执行过程

Server 层和查询共用一套链路，真正改数据和写日志发生在执行器调用 InnoDB 之后。查询链路细节见 `原理/SQL语句执行分析.md`。

### Server 层

* 连接器：建立连接、校验身份
* 查询缓存：update 本身不走缓存；这张表上有更新，整表查询缓存会被清空。命中率低，MySQL 8.0 已移除
* 解析器：词法分析认出 `update`、表名等，再做语法分析
* 预处理器：表、字段是否存在
* 优化器：定执行计划。这里 `id` 是主键，走主键索引
* 执行器：按计划调存储引擎，找到这一行再更新
...
1. 执行器调引擎，按主键索引树找 `id = 1` 这一行
   * 所在数据页已在 Buffer Pool：直接返回给执行器
   * 不在：把整个数据页从磁盘读进 Buffer Pool（不是只读这一行），再返回。页内再用页目录定位记录
2. 执行器对比更新前、更新后的记录。一样就结束；不一样把新旧记录交给 InnoDB
3. 开启事务。InnoDB 更新前先写 **undo log**（记下被改列的旧值）到 Buffer Pool 的 Undo 页；Undo 页改了也要记 **redo log**，保证 undo 自己能持久
4. 更新 Buffer Pool 里的数据页，标成脏页；把这次页修改写进 **redo log buffer**。此时更新就算完成。脏页不立刻刷磁盘，后台线程稍后刷。这就是 **WAL**：先写日志，合适时机再把脏页落盘
5. 这条语句对应的 **binlog** 写入 binlog cache，还不刷硬盘。等事务提交时，把本事务运行过程中的全部 binlog 一起刷盘

### 事务提交（两阶段提交）

双 1 配置（`innodb_flush_log_at_trx_commit=1`、`sync_binlog=1`）下：

* **prepare**：redo log 写入内部 XA 的 XID，事务状态设为 prepare，redo log fsync 到磁盘
* **commit**：把同一个 XID 写入 binlog 并 fsync；再调引擎把 redo log 状态设为 commit。commit 标记不必马上 fsync，只要 binlog 落盘成功，即使 redo 还停在 prepare，崩溃恢复也会提交
* 组提交会把上面的 commit 再拆成 flush / sync / commit 三步，见「两阶段提交」

### 串起来记

1. Server 层走完连接器 / 解析 / 优化，执行器按主键把行从 Buffer Pool（或磁盘）拿出来
2. 新旧值一样则跳过；否则写 undo（旧值）+ undo 页对应的 redo
3. 改内存页为脏页 + 写数据页的 redo（WAL，脏页稍后刷）
4. binlog 进 cache
5. 两阶段提交：redo prepare 刷盘 → binlog 刷盘 → redo 标 commit

整库误删只能靠 binlog 恢复，不能靠 redo log。redo 循环写，已刷脏页对应的记录会被擦掉，不是全量日志。
...
* 内部 XA：binlog 当协调者，InnoDB 当参与者。客户端 `commit` 或 autocommit 时，MySQL 内部开 XA，分两阶段提交
* 两阶段提交：准备+提交
  * 准备：给 redo log 一个 XID，将 redo log 状态设置为 prepare，并持久化到磁盘（`innodb_flush_log_at_trx_commit=1`）
  * 提交：把同一个 XID 写入 binlog 并持久化到磁盘（`sync_binlog=1`），再调用引擎把 redo log 状态设置为 commit。**以 binlog 写成功作为事务提交成功的标志**；commit 标记只 write 到 Page Cache 即可，因为 binlog 已落盘，崩溃恢复也能提交

![1772464793631](image/黑马MySQL/1772464793631.png)

* 异常重启时，扫到 prepare 状态的 redo log，拿里面的 XID 去 binlog 里找
  * binlog 里没有这个 XID：redo 刷了、binlog 还没刷，回滚（对应 redo 已落盘、binlog 未落盘就崩溃）
  * binlog 里有这个 XID：两份都刷完了，提交（对应 binlog 已落盘、redo 还没标 commit 就崩溃）。从库会用这份 binlog，主库也必须提交，主从才一致
* redo log 在事务没提交时也可能被后台线程每秒刷盘；**binlog 必须等事务提交后才能持久化**。没提交就崩溃，binlog 里没有对应 XID，重启会回滚，不会把半成品提交出去
